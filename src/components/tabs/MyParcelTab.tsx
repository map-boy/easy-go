        ...
        {loading ? (
          ...
          <>
            ...
            {activeOrders.map(order => order._type === 'food' ? (
              <FoodOrderCard key={order.id} order={order} onConfirm={() => confirmFoodReceipt(order.id)} confirmingId={confirmingId} />
            ) : (
              <ActiveOrderCard
                key={order.id}
                order={order}
  ...
  return (
    <div style={{ background: 'var(--card)', borderRadius: '16px', overflow: 'hidden', marginBottom: '16px', border: `1px solid ${color}44` }}>
      ...
      <div style={{ padding: '12px 16px' }}>
        ...
        {order.status === 'delivered' && !order.receiver_confirmed && onConfirm && (
          <div style={{ marginTop: '12px', background: 'rgba(139,92,246,0.07)', border: '2px solid rgba(139,92,246,0.3)', borderRadius: '12px', padding: '14px' }}>
            ...
            <p style={{ fontSize: '11px', color: 'var(--text3)', marginBottom: '10px' }}>
              Confirm you received it so the driver gets paid. Driver earns <strong style={{ color: 'var(--yellow)' }}>{Math.round((order.delivery_fee || 0) * 0.7).toLocaleString()} RWF</strong> (70% of delivery fee).
            </p>
            <button
              onClick={onConfirm}
              disabled={confirmingId === order.id}
              style={{ width: '100%', padding: '13px', background: '#8b5cf6', border: 'none', borderRadius: '10px', fontWeight: 800, fontSize: '14px', color: '#fff', cursor: 'pointer', fontFamily: 'Space Grotesk, sans-serif', opacity: confirmingId === order.id ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              ✅ {confirmingId === order.id ? 'Confirming…' : 'I Received My Order'}
            </button>
          </div>
        )}
        ...
        {/* Already confirmed */}
        {order.receiver_confirmed && (
          <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 12px', background: 'rgba(34,197,94,0.07)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '10px' }}>
            <span style={{ fontSize: '16px' }}>✅</span>
            <p style={{ fontSize: '12px', fontWeight: 700, color: 'var(--green)' }}>You confirmed receipt — driver has been paid</p>
          </div>
        )}
