...
import { User, Edit2, Save, LogOut, Info, MessageCircle,
         ...
         Clock, RefreshCw, X, Globe, Shield, FileText, Phone } from 'lucide-react';
...
export function ProfileTab() {
  ...
  useEffect(() => {
    if (profile?.user_category === 'motari' || profile?.role === 'driver') loadDriverInfo();
  ...
  async function loadDriverInfo() {
    if (!profile) return;
    const { data } = await supabase.from('drivers').select('*').eq('user_id', profile.id).maybeSingle();
  ...
  async function loadWallet() {
    ...
    const { data: prof } = await supabase.from('profiles').select('wallet_balance').eq('id', profile.id).single();
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => {
      ...
      setPhotoPreview(url);
      localStorage.setItem(`eg-photo-${profile?.id}`, url);
    };
  ...
  async function switchRole(newRole: 'sender' | 'receiver') {
    ...
    await supabase.from('profiles').update({ role: newRole, user_category: newRole, updated_at: new Date().toISOString() }).eq('id', profile.id);
    await refreshProfile();
  ...
  async function toggleDuty() {
    ...
    await supabase.from('drivers').update({ is_on_duty: newDuty, is_available: newDuty, on_duty_since: newDuty ? new Date().toISOString() : null }).eq('id', driverInfo.id);
    setDriverInfo({ ...driverInfo, is_on_duty: newDuty, is_available: newDuty });
    notify(newDuty ? '🟢 You are now On Duty' : '🔴 You are now Off Duty');
  }

  if (!profile) return <div style={{ textAlign: 'center', padding: '48px 0' }}><div className="spinner" /></div>;

  const isDriver   = profile.user_category === 'motari' || profile.role === 'driver';
  const roleLabel: Record<string, string> = {
    sender: '📤 Sender', receiver: '📥 Receiver', motari: '🏍️ Motari (Driver)', driver: '🏍️ Motari (Driver)',
  };

  // ── reusable fold section ──
  function FoldSection({ icon: Icon, label, open, toggle, children }: any) {
    return (
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <button onClick={toggle} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 18px', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Space Grotesk, sans-serif' }}>
  ...
  return (
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px', background: 'var(--bg)', minHeight: '100%' }}>
      <style>{`@keyframes tt-dot { 0%,100%{opacity:1;transform:scale(1);} 50%{opacity:.3;transform:scale(.6);} }`}</style>

      {msg && (
        <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '10px', padding: '10px 14px', fontSize: '13px', fontWeight: 600, color: 'var(--green)' }}>
          {msg}
        </div>
      )}
      ...
      <FoldSection icon={User} label="My Information" open={showMyInfo} toggle={() => setShowMyInfo(v => !v)}>
        <div style={{ padding: '14px 18px' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '10px' }}>
            {!isEditing ? (
              <button onClick={() => setIsEditing(true)} style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '9px', padding: '6px 12px', cursor: 'pointer', color: 'var(--text)', fontSize: '12px', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600 }}>
                <Edit2 size={12} /> Edit
              </button>
            ) : (
              <button onClick={handleSave} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '9px', padding: '6px 12px', cursor: 'pointer', color: 'var(--green)', fontSize: '12px', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[
                ...
                <div key={f.k}>
                  ...
                  <input className="eg-input" placeholder={f.ph} value={(form as any)[f.k]}
      ...
      {/* ── ROLE (folded) ── */}
      <FoldSection icon={User} label={`Role — ${roleLabel[profile.user_category || profile.role] || profile.role}`} open={showRole} toggle={() => setShowRole(v => !v)}>
        <div style={{ padding: '14px 18px' }}>
          {isDriver && driverInfo ? (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: driverInfo.is_on_duty ? 'rgba(34,197,94,0.06)' : 'rgba(239,68,68,0.05)', border: `1px solid ${driverInfo.is_on_duty ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.15)'}`, borderRadius: '10px' }}>
                <div>
                  <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text)', marginBottom: '2px' }}>{driverInfo.is_on_duty ? '🟢 On Duty' : '🔴 Off Duty'}</p>
                  <p style={{ fontSize: '11px', color: 'var(--text3)' }}>{driverInfo.is_on_duty ? 'You are receiving orders' : 'Toggle to receive orders'}</p>
                </div>
                <button onClick={toggleDuty} style={{ width: '56px', height: '30px', background: driverInfo.is_on_duty ? '#22c55e' : 'var(--border2)', borderRadius: '15px', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background .25s', boxShadow: driverInfo.is_on_duty ? '0 0 12px rgba(34,197,94,0.4)' : 'none' }}>
                  <div style={{ position: 'absolute', top: '4px', left: driverInfo.is_on_duty ? '30px' : '4px', width: '22px', height: '22px', background: '#ffffff', borderRadius: '50%', transition: 'left .25s', boxShadow: '0 1px 4px rgba(0,0,0,0.2)' }} />
                </button>
              </div>
            </>
          ) : (
            <>
              <p style={{ fontSize: '12px', color: 'var(--text3)', marginBottom: '12px' }}>Switch between sender and receiver anytime</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                {[
                  ...
                  return (
                    <button key={r.value} onClick={() => switchRole(r.value as 'sender' | 'receiver')}
                      style={{ background: isActive ? 'rgba(245,197,24,0.1)' : 'var(--bg3)', border: `2px solid ${isActive ? 'rgba(245,197,24,0.45)' : 'var(--border)'}`, borderRadius: '12px', padding: '14px', cursor: 'pointer', textAlign: 'left', transition: 'all .15s' }}>
                      <p style={{ fontSize: '20px', marginBottom: '6px' }}>{r.emoji}</p>
                      <p style={{ fontSize: '13px', fontWeight: 700, color: isActive ? 'var(--yellow)' : 'var(--text)', marginBottom: '3px', fontFamily: 'Space Grotesk, sans-serif' }}>{r.label}</p>
                      <p style={{ fontSize: '11px', color: 'var(--text3)', fontFamily: 'Space Grotesk, sans-serif' }}>{r.desc}</p>
                      {isActive && <p style={{ fontSize: '10px', color: 'var(--yellow)', fontWeight: 700, marginTop: '6px' }}>✓ Current</p>}
                    </button>
                  );
                })}
              </div>
      ...
      {/* ── ABOUT US (folded) ── */}
      <FoldSection icon={Info} label="About Us" open={showAbout} toggle={() => setShowAbout(v => !v)}>
        <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          ...
          <a href="https://easy-go-delta.vercel.app" target="_blank" rel="noreferrer"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>Our Website</span>
            </div>
          ...
          <a href="https://easy-go-delta.vercel.app/privacy" target="_blank" rel="noreferrer"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>Privacy Policy</span>
            </div>
            <ChevronRight size={13} color="var(--text3)" />
          </a>
          ...
          <a href="https://easy-go-delta.vercel.app/terms" target="_blank" rel="noreferrer"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>Terms & Conditions</span>
            </div>
            <ChevronRight size={13} color="var(--text3)" />
          </a>
          ...
          <div style={{ padding: '12px 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
              <Phone size={14} color="var(--yellow)" />
              <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>Contact Us</span>
            </div>
            {[
              ...
              <div key={i.l} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--border)' }}>
          ...
          {/* Short about text */}
          <div style={{ marginTop: '8px', padding: '12px', background: 'var(--bg3)', borderRadius: '10px' }}>
            <p style={{ fontSize: '13px', color: 'var(--text2)', lineHeight: 1.7 }}>
              <strong style={{ color: 'var(--text)' }}>Easy GO</strong> connects senders with reliable motor drivers across Rwanda.
              Making delivery simple, fast, and affordable for everyone.
            </p>
            <p style={{ marginTop: '8px', color: 'var(--yellow)', fontWeight: 700, fontSize: '13px' }}>Your Delivery is our duty. 🇷🇼</p>
          </div>
        </div>
      </FoldSection>
